...
function getFitness(individual){ //calculate fitness value of any individual
    var duration = distanceMatrix[0][individual[1]].duration;
    var total = duration + baseRoute[individual[individual.length-1]].expected_time;
    var hours = new Date(date);

    for(var i=1;i<individual.length-1;i++){
        duration = distanceMatrix[individual[i]][individual[i+1]].duration;
        expected_time = parseFloat(baseRoute[individual[i]].expected_time);
        //get penalties
        penalties = getPenalties(hours,baseRoute[individual[i]]);
    ...
    function getPenalties(date,poi) {
        open = new Date(date.toDateString().split(' ').splice(1,3).join(' ')+' 13:00:00');
        ...
        try{
            ...
            return time >= 1000 && time < 1700 ? 0 : 90 ;
        }
    }
...
function getSchedule(individual){
    ...
    //location User
    schedule.push({poi:baseRoute[0]});
    //---ROUTE TO FIRST POI---
    schedule.push({
        route:distanceMatrix[0][individual[1]].points,
    ...
    for(var i=1;i<individual.length-1;i++){
        ...
        //visit time to POI
        visit_time = {poi:baseRoute[individual[i]]}

        //route to next POI
        next = {route:distanceMatrix[individual[i]][individual[i+1]].points}
        ...
        //lunch time
        if(hours.getTime() >= lunch_start && hours.getTime() < lunch_end){
            lunch_time = {lunch:true,location:baseRoute[individual[i+1]].location};
            setTime(hours,60,lunch_time);
            schedule.push(lunch_time);
        }
    }
    last={poi:baseRoute[individual[individual.length-1]]};
    setTime(hours,baseRoute[individual[individual.length-1]].expected_time,last);
    schedule.push(last);

    return schedule;

    function setTime(date,duration,obj){
        obj.start=date.toTimeString().split(' ')[0];
        date.setTime(date.getTime()+(duration*60*1000));
        obj.end=date.toTimeString().split(' ')[0];
    }
}
