...
function getRandomPositionsMutation(total,start,limit){//get random positions of any array
    var random = new Array(total);
    var i = 0;
    while(i<random.length){
        num = Math.floor(Math.random()*limit+start);
...
function getFitness(individual){ //calculate fitness value of any individual
    var total = baseRoute[individual[individual.length-1]].expected_time;
    var schedule = new Array(individual.length);
    var hours = new Date(date);
    var lunch_start = date.getTime()+(3*3600*1000);
    var lunch_end = lunch_start+(3600*1000);

    for(var i=0;i<individual.length-1;i++){
        const expected_time = parseFloat(baseRoute[individual[i]].expected_time);
        const duration = distanceMatrix[individual[i]][individual[i+1]].duration;

        //get penalties
        const penalties = getPenalties(hours,baseRoute[individual[i]]);
        //POI expected visit time
        schedule[i] = {};
        schedule[i].start = hours.toTimeString().split(":", 2).join(':');
        hours.setTime(hours.getTime()+(expected_time*60*1000));
        schedule[i].end = hours.toTimeString().split(":", 2).join(':');
        //add time duration to arrive to the next poi
        hours.setTime(hours.getTime()+(duration*60*1000));
        hours.setTime(hours.getTime() >= lunch_start && hours.getTime() < lunch_end ? lunch_end : hours.getTime() );
        //TODO : sumar el tiempo de almuerzo :D

        //add total
        total+= expected_time+duration+penalties;
    }
    schedule[schedule.length-1]={};
    schedule[schedule.length-1].start = hours.toTimeString().split(":", 2).join(':');
    hours.setTime(hours.getTime()+(baseRoute[individual[individual.length-1]].expected_time*60*1000));
    schedule[schedule.length-1].end = hours.toTimeString().split(":", 2).join(':');
...
function getPenalties(date,poi) {
    try{
        time = parseInt(date.toTimeString().split(":", 2).join(''));
        open = parseInt(poi.opening_hours.periods[date.getDay()].open.time);
        close = parseInt(poi.opening_hours.periods[date.getDay()].close.time);
        return time >= open && time < close ? 0 : 90 ;
    }catch(err) {
        return 60 ;
    }
}
...
function sort() {//sort population according to Fitness value of individuals
    ...
    if(best.fitness.value > bestIndividual.fitness.value){
        bestIndividual = best;
    }
}
...
function mutation(mutation_probability) {
    function mutate(individual){
        ...
        var positions = getRandomPositionsMutation(Math.floor((route.length-1)/2),1,route.length-1);
...
module.exports = {
    evolve : function (parameters) {
        ...
        baseRoute = parameters.baseRoute;
        //---EVOLVE---
        ...
        var tour = bestIndividual.route;
        var routes = [];
        delete bestIndividual.route;
        for(var i=0;i<tour.length-1;i++){
            routes.push(distanceMatrix[tour[i]][tour[i+1]].points);
        }
        for(var i in tour){
            tour[i]=baseRoute[tour[i]];
        }
        bestIndividual.tour = tour;
        bestIndividual.routes = routes;
