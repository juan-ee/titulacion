...
, Meta = function (id, item, lat, lng, ks) {
  this.id = id;
  this.item = item;
  this.lat = lat;
  this.lng = lng;
...
/* data array of point objects with coordinates { lat: 0, lng: 0 } */
...
Kmeans.prototype.calc = function (data) {
  ...
  for (var k = 0; k < ks; k++) {
    var centroid = {
        lat: result[0].centroids[k].lat
      , lng: result[0].centroids[k].lng
      , k: k
      , items: 0
      , data: {}
      , count: function () {
...
var doNormalize = function () {
  ...
  /* get minimum and maximum lat */
  points.sort(function (a, b) {
    return a.lat - b.lat;
  });

  var x_min = points[0].lat;
  var x_max = points[ps - 1].lat;

  /* get minimum and maximum lng */
  points.sort(function (a, b) {
    return a.lng - b.lng;
  });

  var y_min = points[0].lng;
  var y_max = points[ps - 1].lng;

  /* normalize */
  for (var p = 0; p < ps; p++) {
    var point = points[p];
    point.lat = (point.lat - x_min) / (x_max - x_min);
    point.lng = (point.lng - y_min) / (y_max - y_min);
  }
...
var initMetas = function () {
  ...
  for (var p = 0; p < ps; ++p) {
    ...
    point.k = -1;
    var meta = new Meta(p, point, point.lat, point.lng, ks);
...
var kmeansplusplus = function () {
  ...
  /* 2. For each data point lat, compute D(lat), the distance between lat and the nearest center that has already been
    chosen. */
  for (i = 0; i < ps; ++i)
  ...
  /* 3. Choose one new data point at random as a new center, using a weighted probability distribution where a
    point lat is chosen with probability proportional to D(lat)2. (Repeated until k centers have been chosen.) */
  for (k = 1; k < ks; ++k) {
  ...
  for (var k = 0; k < ks; ++k) {
    ...
    for (var prop in centroid.data) {
      if (centroid.data.hasOwnProperty(prop)) {
        sum_x += centroid.data[prop].lat;
        sum_y += centroid.data[prop].lng;
      }
    }
    if (sum_x > 0 && sum_y > 0) {
      centroid.lat = sum_x / centroid.items;
      centroid.lng = sum_y / centroid.items;
    }
  }
  ...
  var sum = 0;
    for (var p = 0; p < ps; p++) {
      var point = points[p]
        ...
        , dif_x = pow(point.lat - centroid.lat, 2)
        , dif_y = pow(point.lng - centroid.lng, 2)
        ;
      sum += dif_x + dif_y;
    }
  return sum;
...
var kmeans = function () {
  ...
  while (!converged) {
    ...
    for (var p = 0; p < ps; ++p) {
      ...
      sums[k].lat += points[p].lat;
      sums[k].lng += points[p].lng;
    ...
    for (var k = 0; k < ks; ++k) {
      if (sums[k].items > 0) {
        centroids[k].lat = sums[k].lat / sums[k].items;
        centroids[k].lng = sums[k].lng / sums[k].items;
      }
